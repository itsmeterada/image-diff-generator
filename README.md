...
- **Image 2 + マスクアルファ保存**: Image 2にマスクをアルファチャンネルとして適用した画像を保存

### ✨ エッジスムージング
- マスクのエッジを滑らかにするガウシアンブラー機能
- オン/オフ切り替え可能
- 強度調整スライダー（1〜50）
...
6. **Image 2にマスクを適用して保存**（オプション）:
   - 「Save Image 2 with Mask Alpha」ボタンをクリック
   - Image 2のRGB + マスクのアルファチャンネルを持つPNG画像が保存されます

### エッジスムージングの使い方

1. マスクを生成（差分検出またはSAM3）
2. 「Enable Edge Smoothing」にチェックを入れる
3. Strengthスライダーで強度を調整（1〜50、デフォルト: 3）
4. 保存ボタンをクリックするとスムージングが適用されます
...
1. **モデルをロード**:
   ...
   - SAM3未インストール時は「Show Install Guide」ボタンでインストール手順を確認
...
3. **テキストプロンプトを入力**:
   ...
   - 複数のオブジェクトはカンマで区切る（例: `people, people shadow`）
   - デフォルト: `people, people shadow`
...
6. **閾値を調整**（オプション）:
   - Thresholdスライダーで検出感度を調整（0.0〜1.0、デフォルト: 0.50）
